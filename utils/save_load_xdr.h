...
/*
template<typename xdr_type, uint32_t BUF_SIZE = 65536>
void save_xdr_to_file_fast(const xdr_type& value, const char* filename, const size_t prealloc_size = 64000000)
{
...
void save_list_xdr_to_file_fast(const xdr_list_type& value, const char* filename, const size_t prealloc_size = 64000000)  {
...
void save_list_xdr_to_file_fast(const xdr_list_type& value, utils::unique_fd& fd, unsigned char* buffer, const unsigned int BUF_SIZE) {
	...
	while (list_idx < list_size) {
		...
		if (aligned_buf_size - buf_idx < next_sz) {
			...
			detail::flush_buffer(fd, aligned_buf, write_amount);
