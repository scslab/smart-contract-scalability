...
GroundhogPersistentStateDB::get_committed_value(const AddressAndKey& a)
{
...
void
do_nothing_if_merge_groundhog(const GroundhogPersistentStateDB::value_t& value)
{}

std::optional<RevertableObject::DeltaRewind>
GroundhogPersistentStateDB::try_apply_delta(const AddressAndKey& a,
                                 const StorageDelta& delta)
{
    ...
    if (!res) {
        ...
        root->template insert<&do_nothing_if_merge_groundhog>(a, state_db.get_gc(), current_timestamp, state_db.get_storage());
...
struct GroundhogUpdateFn
{
    GroundhogPersistentStateDB::trie_t& main_db;
    uint32_t current_timestamp;

    using prefix_t = GroundhogPersistentStateDB::prefix_t;
    using index_trie_t = ModifiedKeysList::map_t;
...
void
GroundhogPersistentStateDB::commit_modifications(const ModifiedKeysList& list)
{
    auto ts = utils::init_time_measurement();

    GroundhogUpdateFn update(state_db, current_timestamp);

    std::printf("parallel modify before %lf\n", utils::measure_time(ts));

    uint32_t grain_size = std::max<uint32_t>(1, list.size() / 1000);
    list.get_keys()
        .parallel_batch_value_modify_const<GroundhogUpdateFn, prefix_t::len()>(
            update, grain_size);
...
void
GroundhogPersistentStateDB::rewind_modifications(const ModifiedKeysList& list)
{
    GroundhogRewindFn rewind(state_db, current_timestamp);

    list.get_keys()
        .parallel_batch_value_modify_const<GroundhogRewindFn, prefix_t::len()>(rewind,
                                                                      1);
    state_db.hash_and_normalize(0);
    state_db.do_gc();
}
...
Hash
GroundhogPersistentStateDB::hash()
{
