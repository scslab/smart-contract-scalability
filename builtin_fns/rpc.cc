    	...
    	uint32_t response_max_len)
{
	...
	{
		auto h = runtime.template load_from_memory_to_const_size_buf<Hash>(target_addr);
		auto calldata = runtime.template load_from_memory<xdr::opaque_vec<>>(call_data_offset, call_data_len);
		...
		client.execute_call(
			RpcCall(h, calldata), 
			[request_id] (xdr::call_result<RpcResult> cb) {

				if (!cb)
				{
					ThreadlocalContextStore::get_timer().notify(std::nullopt, request_id);
				}
				else {
					ThreadlocalContextStore::get_timer().notify(*cb, request_id);
				}
			});

		auto result_opt = ThreadlocalContextStore::get_timer().await(request_id);
