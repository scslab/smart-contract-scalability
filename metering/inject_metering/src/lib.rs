...
#[no_mangle]
pub extern "C" fn add_metering_ext(data_ptr: *mut u8, len: u32) -> metered_contract {
	...
	let mut vec = match match std::panic::catch_unwind(|| {
		...
		Ok(v) => v,
		Err(_) => { return metered_contract::null(); },
    } {
		Some(v) => v,
		None => { return metered_contract::null(); },
    };
...
pub extern "C" fn free_metered_contract(contract: metered_contract) {
	...
	unsafe {
		drop(Vec::from_raw_parts(contract.data, contract.len as usize, contract.capacity as usize));
	}
}
