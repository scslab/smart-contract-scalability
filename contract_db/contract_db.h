    ...
    : public utils::NonMovableOrCopyable
{
    struct value_struct
    {
        ...
        void copy_data(std::vector<uint8_t>& buf) const
        {
            if (contract)
            {
                buf.insert(buf.end(), contract->bytes.data(), contract->bytes.data() + contract->bytes.size());
            }
        }
    };
    ...
    // address -> contract: where contracts are deployed
    contract_map_t addresses_to_contracts_map;
    // hash (of contract) -> contract: allow lookup of contracts by hash (useful when deploying contracts)
    std::map<Hash, verified_script_ptr_t> hashes_to_contracts_map;
    ...
    VerifiedScriptView get_script_by_hash(const Hash& hash) const;

    VerifiedScriptView get_script_by_address(
